...
class DownloadService : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        startInForeground("Downloading...", 0)
        acquireWakeLock()
        isRunning = true
        ...
        scope.launch {
            try {
                ...
                val request = buildRequest(url, quality, fmt, useAria2)
                ...
                notifyDone("Download complete", "Saved in Download/Video Downloads")
                ...
                if (stoppedByNetwork) {
                    ...
                    progressListener?.invoke(-1, "Stopped: WiFi disconnected")
                } else {
                    val msg = e.message?.take(120) ?: "Unknown error"
                    notifyDone("Download failed", msg)
                    progressListener?.invoke(-1, "Failed: $msg")
                }
            } finally {
                isRunning = false
                unwatchNetwork()
    ...
    // ---------- request building (same logic as desktop app) ----------

    private fun buildRequest(
        url: String, quality: String, fmt: String, useAria2: Boolean
    ): YoutubeDLRequest {
